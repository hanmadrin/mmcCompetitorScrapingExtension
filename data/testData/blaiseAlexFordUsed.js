(async () => {
    const blaisealexanderfordofmansfieldSingle = async (start=0) => {
        const res = await fetch("https://www.blaisealexanderfordofmansfield.com/api/widget/ws-inv-data/getInventory", {
            "headers": {
                "accept": "*/*",
            ...
            "body": JSON.stringify({
                ...
                "inventoryParameters": {
                    "accountId": "blaisealexandermansfieldfordfd",
                    "start": [
                        `${start}`
                    ]
                },
        ...
        for (const item of data.inventory) {
            listings.push({
                "Dealer Group": "A",
                "Dealership Name": "Blaise Alexander Ford of Mansfield",
                "Used or New": "Used",
                "URL": `https://www.blaisealexanderfordofmansfield.com${item.link}`,
                "Year": item.year,
                "Make": item.make,
                "Model": item.model,
                "Trim": item.trim,
                "Price": item.trackingPricing.askingPrice,
    ...
    const {pageSize, totalCount} = await blaisealexanderfordofmansfieldSingle();
    ...
    for (let page = 0; page < totalPages; page++) {
        ...
        const {listings} = await blaisealexanderfordofmansfieldSingle(start);
